...
def rename_f(src, dest):
    if not os.path.exists(src):
        raise ValueError("%s does not exist" % src)
    if not os.access(os.path.dirname(src), os.W_OK):
        raise ValueError("Don't have write perm on source dir")
    if os.path.exists(dest):
        os.remove(dest)
    distlog.info("Moving %s to %s", src, dest)
...
class seal_build(du_build):
  ...
  def package(self):
    ...
    for jar in glob.iglob(os.path.join(sbt_target_dir, '*.jar')):
      ...
      rename_f(jar, dest_jar_path)
    # copy the Avro schemas as well
    schema_dir = os.path.join(self.build_purelib, 'seal', 'lib', 'io', 'schemas')
    if not os.path.isdir(schema_dir):
      os.makedirs(schema_dir)
    sbt_schema_dir = os.path.join(ProjectRoot, 'sbt', 'temp', 'avschemas')
    for schema in glob.iglob(os.path.join(sbt_schema_dir, '*.avsc')):
        rename_f(schema, os.path.join(schema_dir, os.path.basename(schema)))
...
if __name__ == '__main__':
    ...
    ##      download_url=DOWNLOAD_URL,
          ...
          package_data={ 'seal.lib.io.schemas': '*.avsc' },
          cmdclass={
              "bdist": seal_bdist,
              "build": seal_build,
              "clean": seal_clean,
