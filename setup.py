...
class seal_build(du_build):
  ...
  def finalize_options(self):
    ...
    if os.environ.has_key('PARQUETMR_JAR'):
        if os.path.exists(os.environ['PARQUETMR_JAR']):
            self.parquetmr_jar = os.environ['PARQUETMR_JAR']
        else:
            raise ValueError("PARQUETMR_JAR env variable set to %s, but the file doesn't exist" % os.environ['PARQUETMR_JAR'])
    else:
        path = self._find_parquet_mr_jar(seal_build_parquet_mr.parquetmr_autobuild_dir)
        if path:
            self.parquetmr_jar = path
    if self.parquetmr_jar:
        print >> sys.stderr, "Bundling parquetmr jar", self.parquetmr_jar
  ...
  def package(self):
    ...
    self._package_parquetmr(jar_dir)
...
class seal_build_parquet_mr(seal_build_bundled_stuff):
    ...
    parquetmr_autobuild_dir = os.path.join(seal_build_bundled_stuff.bundled_dir, "parquetmr_autodir")

    def _edit_build_sbt(self, hadoop_ver):
        distlog.info("Setting hadoop version to %s in build.sbt for parquet-mr", hadoop_ver)
        build_sbt = os.path.join(self.parquetmr_autobuild_dir, 'build.sbt')
    ...
    def run(self):
        ...
        with chdir(self.parquetmr_autobuild_dir):
            distlog.debug("chdir'd into %s", self.parquetmr_autobuild_dir)
            distlog.info("Running 'sbt assembly' in %s" % self.parquetmr_autobuild_dir)
...
if __name__ == '__main__':
    ...
    ##      download_url=DOWNLOAD_URL,
          ...
          cmdclass={
              ...
              "build_parquetmr": seal_build_parquet_mr,
