...
#ifdef BWT_EXPORT_LIBRARY_FUNCTIONS
/* 
   FIXME the following functions are here since they (probably!)
   depend on static functions defined in this file. It would be
   cleaner if we could move them in an ad-hoc file.
 */
...
// __pairing_aux_memory(position, seq_flag)
#define __pairing_aux_memory(u,v) do {					\
    bwtint_t l = ((v)>>32) + p[(v)&1]->len - ((u)>>32);			\
    if ((u) != (uint64_t)-1 && (v)>>32 > (u)>>32 && l >= max_len	\
        && ((ii->high && l <= ii->high_bayesian) || (ii->high == 0 && l <= opt->max_isize))) \
      {									\
	uint64_t s = p[(v)&1]->aln[(uint32_t)(v)>>1].score + p[(u)&1]->aln[(uint32_t)(u)>>1].score; \
	s *= 10;							\
	if (ii->high) s += (int)(-4.343 * log(.5 * erfc(M_SQRT1_2 * fabs(l - ii->avg) / ii->std)) + .499); \
	s = s<<32 | (uint32_t)hash_64((u)>>32<<32 | (v)>>32);		\
	if (s>>32 == o_score>>32) ++o_n;				\
...
#define __pairing_aux2_memory(q, w) do {				\
    const bwt_aln1_t *r = p[(w)&1]->aln + ((uint32_t)(w)>>1);         \
    (q)->extra_flag |= SAM_FPP;						\
    if ((q)->pos != (w)>>32 || (q)->strand != r->a) {                   \
      (q)->n_mm = r->n_mm; (q)->n_gapo = r->n_gapo; (q)->n_gape = r->n_gape; (q)->strand = r->a; \
...
int bwa_cal_pac_pos_pe_memory(bwt_t *const bwt[2], int n_seqs, bwa_seq_t *seqs[2], 
  ...
  for (i = 0; i != n_seqs; ++i) {
    ...
    for (j = 0; j < 2; ++j) {
      ...
      // generate SE alignment and mapping quality
      //bwa_aln2seq(n_aln, d->aln[j].a, p[j]);
      max_n_aln = (p[j]->n_aln > max_n_aln)? p[j]->n_aln : max_n_aln;

      /* FIXME: bwa_aln2seq makes a probabilistic selection of best aln */
