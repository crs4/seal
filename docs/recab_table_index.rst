...
To run RecabTable, launch ``bin/seal_recab_table``.  You will need to provide
input and output paths, and a database of known variation sites in ROD or VCF
format.  For example,

::

  ./bin/seal_recab_table --vcf-file dbsnp.vcf sam_directory recab_table_output
...
TableRecalibration use::
  ...
  ./bin/seal_recab_table_fetch recab_table_output recab_table.csv

or::

  ./bin/seal_recab_table_fetch recab_table_output > recab_table.csv

The utility `seal_recab_table_fetch` simply copies the data generated by
...
``seal_recab_table`` follows the normal Seal usage convention.  See the section
