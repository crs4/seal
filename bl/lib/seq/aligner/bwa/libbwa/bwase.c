		...
		for (k = 0; k < n_aln; ++k) {
			...
			if (q->l - q->k + 1 <= rest) {
				...
				for (l = q->k; l <= q->l; ++l) {
					...
					s->multi[z].n_gapo = q->n_gapo;
					s->multi[z].n_gape = q->n_gape;
					s->multi[z].n_mm = q->n_mm;
					s->multi[z].score = q->score;
#else
		...
		for (i = 0; i < n_seqs; ++i) {
			...
			for (j = 0; j < s->n_multi; ++j) {
				...
				q->cigar = refine_gapped_core(bns->l_pac, ntpac, s->len, q->strand? s->rseq : s->seq, &q->pos,
											  (q->strand? 1 : -1) * (q->n_gapo + q->n_gape), &n_cigar, 0);
#else
				if (q->gap == 0) continue;
				free(q->cigar);
				q->cigar = refine_gapped_core(bns->l_pac, ntpac, s->len, q->strand? s->rseq : s->seq, &q->pos,
...
#ifdef BWT_EXPORT_LIBRARY_FUNCTIONS
					printf(",%d;", q->n_gapo + q->n_gape + q->n_mm);
#else
					printf(",%d;", q->gap + q->mm);
#endif
				}
...
#ifdef BWT_EXPORT_LIBRARY_FUNCTIONS
void bwa_refine_gapped_memory(const bntseq_t *bns, ubyte_t *pacseq, int n_seqs, bwa_seq_t *seqs) {
  int i, j;
  kstring_t *str;
  ...
  free(str->s); free(str);
	...
	// correct for trimmed reads
	/* BWA has a check before calling this function:
	if (!ntbns) // trimming is only enabled for Illumina reads
	In any case, bwa_correct_trimmed includes a check so that if the 
	sequence isn't trimmed it won't do anything.  It should be safe
	to run.
	*/
	for (i = 0; i < n_seqs; ++i)
